...
class GazLSTM(nn.Module):
    def __init__(self, data):
        ...
        self.use_robert = data.use_robert
        self.use_seftlexion = data.HP_use_seftlexion
        ...
        if self.use_bert:
            if self.use_robert:
                char_feature_dim = char_feature_dim + 1024#768
            else:
        ...
        if self.use_bert:
            if self.use_robert:
                self.bert_encoder = BertModel.from_pretrained('./NER/chinese_roberta_wwm_large_ext')
            else:
                self.bert_encoder = BertModel.from_pretrained('./NER/bert-base-chinese')
    ...
    def get_tags(self,gaz_list, word_inputs, biword_inputs, layer_gaz, gaz_count, gaz_chars, gaz_mask_input, gazchar_mask_input, mask, word_seq_lengths, batch_bert, bert_mask):
        ...
        if self.use_char:
            ...
            gaz_embeds = gaz_embeds_d.data.masked_fill_(gaz_mask.data.bool(), 0)  #(b,l,4,g,ge)=（batch_size, 序列长度也就是句子长度，4（bmes）,max_gaz_num最多gaz的个数，每个词对应的embedding）  ge:gaz_embed_dim


        if self.use_count: #根据词频计算权重，count时词频
            count_sum = torch.sum(gaz_count, dim=3, keepdim=True)  #(b,l,4,gn)
            count_sum = torch.sum(count_sum, dim=2, keepdim=True)  #(b,l,1,1)

            weights = gaz_count.div(count_sum)  #(b,l,4,g) weights是词的词频矩阵
            weights = weights*4
