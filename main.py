...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):
    ...
    data.build_gaz_file(gaz_file)# gaz也就是词典，这里是从gaz_file里读取所有词汇，构建词汇列表
    data.build_gaz_alphabet(train_file,count=True) #利用构建的gaz词典，提取train_file中的每一句话，可能形成的词汇进行处理如“中国语言学” 包括【中国，中国语，语言，语言学...】
    data.build_gaz_alphabet(dev_file,count=True)
    data.build_gaz_alphabet(test_file,count=True)
    data.fix_alphabet() ####?? 关闭资源（未搞懂关的是什么资源）
    return data
...
def set_seed(seed_num=1023):#设置随机种子，保证每一次生成的随机数都是一样的
    random.seed(seed_num)
    torch.manual_seed(seed_num)
    np.random.seed(seed_num)

def write_result(test_texts, gold_results, pred_results):
    f = open('./data/bio_results.txt', 'w')
    # print("write error result \n")
    for test_text, gold_result, pred_result in zip(test_texts, gold_results, pred_results):
        if(gold_result == pred_result):
            continue
        for test_text_char, gold_result_char, pred_result_char in zip(test_text[0], gold_result, pred_result):
            f.write(test_text_char + " " + gold_result_char + " "+ pred_result_char )
            if gold_result_char != pred_result_char:
                f.write(" ############")
            f.write("\n")
        f.write("\n")
    f.close()
...
def evaluate(data, model, name):
    ...
    if name == 'test':
        write_result(data.test_texts,gold_results,pred_results)
...
def train(data, save_model_dir, logfile=None , seg=True ):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if end%500 == 0:
                ...
                if logfile != None:
                    logfile.write(("     Instance: %s; Time: %.2fs; loss: %.4f; acc: %s/%s=%.4f\n"%(end, temp_cost, sample_loss, right_token, whole_token,(right_token+0.)/whole_token)))
                sys.stdout.flush()
                sample_loss = 0
        ...
        if logfile != None:
            logfile.write(("Epoch: %s training finished. Time: %.2fs, speed: %.2fst/s,  total loss: %s\n"%(idx, epoch_cost, train_num/epoch_cost, total_loss)))
        ...
        if seg:
            ...
            if logfile != None:
                logfile.write(("Dev: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f\n"%(dev_cost, speed, acc, p, r, f)))
        else:
            current_score = acc
            print(("Dev: time: %.2fs speed: %.2fst/s; acc: %.4f"%(dev_cost, speed, acc)))
            if logfile != None:
                logfile.write(("Dev: time: %.2fs speed: %.2fst/s; acc: %.4f\n"%(dev_cost, speed, acc)))

        if current_score > best_dev:
            if seg:
                print( "Exceed previous best f score:", best_dev)
                if logfile != None:
                    logfile.write( "Exceed previous best f score:" + str(best_dev)+"\n")

            else:
                print( "Exceed previous best acc score:", best_dev)
                if logfile != None:
                    logfile.write( "Exceed previous best acc score:" + str(best_dev) +"\n")
        ...
        if seg:
            ...
            if logfile != None:
                logfile.write(("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f: %.4f\n"%(test_cost, speed, acc, p, r, f)))
        else:
            current_test_score = acc
            print(("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f"%(test_cost, speed, acc)))
            if logfile != None:
                logfile.write(("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f\n"%(test_cost, speed, acc)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default="data/ccks2018/train-bio.txt")
    parser.add_argument('--dev', default="data/ccks2018/dev-bio.txt")
    parser.add_argument('--test', default="data/ccks2018/test-bio.txt")
    ...
    parser.add_argument('--model_type', default='lstm') #可以设置lstm  cnn  transformer三种
    parser.add_argument('--drop', type=float, default=0.4)

    parser.add_argument('--use_biword', dest='use_biword',type=int, default=True) #是否使用bichar对字符进行嵌入
    # parser.set_defaults(use_biword=False)
    parser.add_argument('--use_char', dest='use_char',type=int, default=False) #推测是将BMES统计出来的词语，再拆成char进行处理，（use_char=false就是无法把词嵌入成向量，只好把字符嵌入成向量在合并用来代表一个词）
    # parser.set_defaults(use_biword=False)
    parser.add_argument('--use_count',type=int, default=True) #是否使用BMES中的词合并时是否使用词频
    parser.add_argument('--use_bert',type=int, default=False)
    parser.add_argument('--use_robert',type=int, default=True) #当use_robert模型将使用Robert同时会自动把use_bert置为True
    parser.add_argument('--use_seftlexion',type=int, default=True) #是否使用seftlexion部分，也就是是否使用BMES表示的那一部分
    parser.add_argument('--log_file', default="--")
    args = parser.parse_args()#args包含了模型的所有默认参数
    seed_num = args.seed
    set_seed(seed_num)#设置随机种子
    logfile = None
    if args.log_file != "--":
        logfile = open(args.log_file + ".log","a")
    train_file = args.train #获取训练集的地址
    dev_file = args.dev #获取验证集的地址
    test_file = args.test #获取测试集的地址
    raw_file = args.raw #该变量没有使用
    ...
    if args.seg.lower() == "true":
        ...
        seg = False
    status = args.status.lower() #status有三种，train,dev,test

    save_model_dir = args.modelpath+args.modelname #save_model_dir是存放模型的路径
    save_data_name = args.savedset
    if args.log_file != "--":
        save_data_name = args.log_file+ ".data"
    gpu = torch.cuda.is_available() #判断gpu环境是否可用

    char_emb = "./NER/prer_train/gigaword_chn.all.a2b.uni.ite50.vec" #单字符的预训练vec编码 里面是一个单字符-一个向量
    bichar_emb = "./NER/prer_train/gigaword_chn.all.a2b.bi.ite50.vec" #双字符的预训练vec编码 里面是一个双字符-一个向量
    gaz_file = "./NER/prer_train/ctb.50d.vec" #词典向量预训练集 里面是一个词汇-一个向量

    sys.stdout.flush() # 显示地让缓冲区的内容输出

    if status == 'train':
        if os.path.exists(save_data_name): #判断是否存在缓存文件，有缓存文件就加载Saveset为data对象
            print('Loading processed data')
            with open(save_data_name, 'rb') as fp:
                data = pickle.load(fp)
            data.HP_num_layer = args.num_layer # 对data对象中的某些变量重新赋值
            ...
            data.HP_use_seftlexion = args.use_seftlexion
            ...
            data.use_robert = args.use_robert
            if data.use_robert == True:
                data.use_bert = True
        else:
            data = Data() #生成data,并初始化
            data.HP_gpu = gpu
            data.HP_use_char = args.use_char
            data.HP_use_seftlexion = args.use_seftlexion
            data.HP_batch_size = args.batch_size
            ...
            if data.use_robert == True:
                data.use_bert = True
            data_initialization(data, gaz_file, train_file, dev_file, test_file) #将词典信息读入data并读入便于划分词汇
            data.generate_instance_with_gaz(train_file,'train') #生成带有词汇信息的数据
            data.generate_instance_with_gaz(dev_file,'dev')
            data.generate_instance_with_gaz(test_file,'test')
            data.build_word_pretrain_emb(char_emb) #读取基于字符预训练向量
            data.build_biword_pretrain_emb(bichar_emb)
            data.build_gaz_pretrain_emb(gaz_file)

            print('Dumping data')
            with open(save_data_name, 'wb') as f:
                pickle.dump(data, f) #将对象保存到文件中
            set_seed(seed_num)#设置随机种子
        data.show_data_summary()
        f = open("word_count2018.txt", 'w',encoding='utf-8')
        f.write(str(data.gaz_count))
        f2 = open("word_id2018.txt", 'w',encoding='utf-8')
        f2.write(str(data.gaz_alphabet.instance2index))
        # np.save('word_count.npy', data.gaz_count)  # 注意带上后缀名
        # np.save('word_id.npy', data.gaz.ent2id)  # 注意带上后缀名
        print("################len: ",len(data.gaz_count))
        print('data.use_biword=',data.use_bigram)
        train(data, save_model_dir,logfile, seg) #模型训练
    elif status == 'test':
        print('Loading processed data')
        with open(save_data_name, 'rb') as fp:
            data = pickle.load(fp)
