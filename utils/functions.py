...
def read_instance_with_gaz(num_layer, input_file, gaz, word_alphabet, biword_alphabet, biword_count, char_alphabet, gaz_alphabet, gaz_count, gaz_split, label_alphabet, number_normalized, max_sent_length, use_robert = False, char_padding_size=-1, char_padding_symbol = '</pad>'):
    if use_robert == True:
    ...
    for idx in range(len(in_lines)):
        line = in_lines[idx]
        if len(line) > 2: #line>2时表示不是只有\n，改行有数据因为一个  字+空格+BIO标注，长度至少是3
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words)>0): #max_sent_length<0代表没有长度限制，否则words的个数应该在0到max_sent_length之间
                gaz_Ids = []
                layergazmasks = []
                gazchar_masks = []
                w_length = len(words)
                ...
                #gazs BMES [                    gazs_count[                                        gaz_char_Id [
                #           [语言,语言学],                  [语言-词频,语言学-词频] ,                              [[语id,言id],[语id,言id,学id]],
                #           [中国语言],                     [.......]
                #           [国语，中国语]，
                #           [语]]                           ]
...
def build_pretrain_embedding(embedding_path, word_alphabet, embedd_dim=100, norm=True):    
    ...
    for word, index in word_alphabet.instance2index.items():
        if word in embedd_dict:
            ...
            perfect_match += 1
        elif word.lower() in embedd_dict: #变换大小写之后，预训练中有的词汇
            if norm:
