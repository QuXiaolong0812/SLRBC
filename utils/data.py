...
class Data:
    ...
    def show_data_summary(self): # 数据设置打印
    ...
    def build_gaz_file(self, gaz_file):
        ...
        if gaz_file:
            ...
            print ("Load gaz file: ", gaz_file, " total size（词典中的词汇个数）:", self.gaz.size())
        else:
    ...
    def build_gaz_alphabet(self, input_file, count=False):
        ...
        for line in in_lines:
            if len(line) > 3:#小于3时是换行符
                ...
                entitys = [] ##猜测是句子中的所以可能的单词组合，也就是所有可能的词
                ...
                word_list = []
        print("gaz alphabet size:","（数据", input_file,"中包含词汇的个数）", self.gaz_alphabet.size())
